...
Изменить баланс Кредитного рейтинга на [число] @username - Изменить рейтинг лояльности
Добавить Очки Клуба [число] @username - Добавить очки клуба
Убрать Очки Клуба [число] @username - Убрать очки клуба
Добавить Рейтинг лояльности [число] @username - Добавить рейтинг лояльности
Убрать Рейтинг лояльности [число] @username - Убрать рейтинг лояльности
...
@dp.message(IsAdmin(), F.text.startswith("Добавить Рейтинг лояльности"))
async def add_credit_rating(message: Message):
    await adjust_points(message, "credit_rating", 1)

@dp.message(IsAdmin(), F.text.startswith("Убрать Рейтинг лояльности"))
...
async def adjust_points(message: Message, field: str, multiplier: int):
    pattern = rf"(Добавить|Убрать) {'Очки Клуба' if 'Очки' in message.text else 'Рейтинг лояльности'}\s+([-+]?\d*\.?\d+)\s+@(\w+)"
    match = re.search(pattern, message.text)
    
    if not match:
        await message.reply(f"❌ Неправильный формат команды. Пример: {'Добавить' if multiplier > 0 else 'Убрать'} {'Очки Клуба' if field == 'club_points' else 'Рейтинг лояльности'} 50 @username")
        return
...
async def show_bank(message: Message):
    ...
    response = (
        ...
        f"💹 Рейтинг лояльности: {credit_rating:.2f}"
    )
...
async def update_credit_rating(message: Message):
    ...
    logger.info(f"Обновлен рейтинг лояльности для @{username}: {rating}")
    await message.reply(f"✅ Рейтинг лояльности пользователя @{username} изменен на {rating:.2f}")
